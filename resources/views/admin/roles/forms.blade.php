            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    @foreach($roles as $role)
                            {{ $role->name }}<input name="role[]" value="{{ $role->id }}" type="checkbox"

                            @if(isset($data))
                            {{ in_array($role->id,SiteHelper::usersRoleIds($data['user'])) ? 'checked="checked"':'' }}
                            @elseif(!empty(old('role')))
                            {{ in_array($role->id,old('role')) ? 'checked="checked"' : '' }}
                            @endif >
                    @endforeach
                    @endif
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="select2-purple">
                            <select name="status" class="form-control">
                                ...
                                <option value="1" {{ SiteHelper::checked_values('select','status',isset($data)??true,
                                isset($data)?$data['user']->status:null,1) }}>Active</option>
                                <option value="0" {{ SiteHelper::checked_values('select','status',isset($data)??true,isset($data)?$data['user']->status:null,0) }}>In-active</option>
                                <option value="2" {{ SiteHelper::checked_values('select','status',isset($data)??true,isset($data)?$data['user']->status:null,2) }}>suspense</option>
                            </select>
                        </div>
                    </div>
                </div>
